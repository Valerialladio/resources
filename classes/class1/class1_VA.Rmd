---
title: "Class 1 - Methods 2 Valeria Alladio"
...
knitr::opts_knit$set(root.dir = "C:/Users/franc/Downloads/ROS-Examples-master/ROS-Examples-master")
...
```{r}
# Load data
hibbs <- read.table("C:/Users/franc/Downloads/ROS-Examples-master/ROS-Examples-master/ElectionsEconomy/data/hibbs.dat", header = TRUE)
...
It means to produce data so that if follows a certain distribution. So, instead of observing data I am creating them so that they correspond to my model. 
In the below case a gaussian one
...
simulated <- rnorm(200, 13, 6)

#visualising data
hist(simulated, breaks=200, main="Simulated Data")

#Calculating empirical mean and sd

e_sim_mean <- mean(simulated)
e_sim_sd <- sd(simulated)
...
#I generate x values in range
#seq(from, to, by, length.out, along.with)
generated_x <- seq(from = 0, to = 4, length.out = 100)

#I calculate from them the y, following model perfectly
generated_y <- 30 + 10*generated_x 

#Check
plot(generated_x, generated_y)

#Now I add the error so that it is normal
error <- rnorm(100, 0, 3.9)

y2 <- generated_y + error
...
error2 <- rnorm (100, 0, 10)

y3 <- generated_y + error2

plot(generated_x, y3)
#more scattered data, naturally
...
```{r additional_step, echo=FALSE}
#I arrange simulated data in df

s_df <- data.frame(generated_x, y2, y3)

#Now I fit the regression models
M2 <- stan_glm(y2 ~ generated_x, data = s_df)

M3 <- stan_glm(y3 ~ generated_x, data = s_df)

print(M2)
print(M3)

#I plot the two scatter, plus regression
ggplot(s_df, aes(x= generated_x, y=y2))+
  geom_point(color="steelblue")+
  labs(title = "Simulated data and their regression")+
  geom_smooth(method = "stan_glm", se = TRUE, color = "red")+
  theme_light()+
  theme(plot.title = element_text(hjust=0.5))

ggplot(s_df, aes(x=generated_x, y=y3))+
  geom_point(color="steelblue")+
  labs(title = "Simulated data 2 and their regression")+
  geom_smooth(method = "stan_glm", se = TRUE, color = "red")+
  theme_light()+
  theme(plot.title=element_text(hjust = 0.5))
...
That they follow a normal distribution centered around 13, with a standard deviation (value of sigma) of 6. 
...
They are different because they were approximated to fit a normal distribution.
...
How grades in school for scientific subjects can predict enrollment in STEM faculties

#### (b) Exploring associations
How age, place of residence, gender and income can predict voting trend

#### (c) Extrapolation
Difference in enrollment in STEM subjects for women across generations

#### (d) Causal inference
How much heavy water one can drink without dying
...
I want the trends in percentage of girls'names ending in each letter.

```{r ex_2.3_data, echo=FALSE}

names <- read.csv("C:/Users/franc/Downloads/ROS-Examples-master/ROS-Examples-master/Names/data/allnames_clean.csv")
a_names <- names

```


```{r ex_2.3_filter, echo=FALSE}
#I only take the girls
names<- names %>%
  filter(sex== "F")
```
#IMPORTANT!!
...
```{r ex_2.3_histograms, echo=FALSE}
#For my own edification I make histogram of last letters of girls'names in 1956 and another in 2006, faceting them
#I need to separate by last letter of name, 

names1 <- names %>%
  group_by(grp = substring(name, nchar(name))) %>%
  summarise(tot_56_ll = sum(X1956),
            tot_06_ll = sum(X2006))

#to have percentages I need
tot_g_56 <- sum(names1$tot_56_ll)
tot_g_06 <- sum(names1$tot_06_ll)
#I add new columns to the df
names1 <- names1 %>%
  mutate(perc_56 = ((tot_56_ll/tot_g_56)*100),
         perc_06 = ((tot_06_ll/tot_g_06)*100))
#PLots
p1 <- names1 %>%
  ggplot(aes(x= grp, y= perc_56))+
  geom_bar(stat= "identity", fill= "lightblue", color="steelblue")+
  labs(title = "Last letter of girls'names in 1956", 
       x = "Letter", 
       y = "Percentage of girls born")+
  theme_light()

p2 <- names1 %>%
  ggplot(aes(x= grp, y= perc_06))+
  geom_bar(stat = "identity", fill="lightblue", color="steelblue")+
  labs(title = "Last letter of girls'names in 2006",
       x = "Letter",
       y = "Percentage of girls born")+
  theme_light()
...
p3 <-
  ...
  group_by(last) %>% 
  summarise(sum = sum(`X1956`)) %>% 
  ungroup() %>% 
  mutate(percent = 100 * sum / sum(sum)) %>% 
  
  ggplot(aes(x = last)) +
  geom_col(aes(y = percent), fill = "pink") +
  scale_y_continuous("Percentage of girls born", breaks = 0:1 * 10, 
                     expand = expansion(mult = c(0, 0.05))) +
  labs(subtitle = "Last letter of girls' names in 1956",
       x = NULL) +
  theme_light()
...
p4 <- names %>%
  ...
  mutate(last = str_sub(name, -1)) %>%
  group_by(last) %>%
  summarise(sum = sum(`X2006`)) %>%
  ungroup() %>%
  mutate(percent =100 * sum/sum(sum)) %>%
  
  ggplot(aes(x= last)) +
  geom_col(aes(y=percent), fill = "pink") +
  scale_y_continuous("Percentage of girls born", 
                     breaks =0:1 *10,
                     expand = expansion(mult =c(0, 0.05))) +
  labs(subtitle = "Last letter of girls'names in 2006",
       x = NULL) +
  theme_light()
...
```
```{r 2.3_remove_X-es, echo=FALSE}
#I need to remove the X from the column names

#I DEFINE A FUNCTION

destroyX = function(es) {  #Functiondefined as taking d.f. as variable "es"
  f = es        
  #Not to touch original df we take a copy of it and assign to the variable f
  
  for (col in c(2:ncol(f))){  
    #for each column in dataframe, from the second, to the last
    
    if (startsWith(colnames(f)[col], "X") == TRUE)  { 
      #if starts with 'X' ..
      
      colnames(f)[col] <- substr(colnames(f)[col], 2, 100) 
      #get rid of it with substr, where 2 is starting position of new name, and 100 ending position (large enough for whole column name)
    }
  }
  assign(deparse(substitute(es)), f, inherits = TRUE) 
  #assign corrected data to original name (es)
  #inherits = TRUE ensures that the new data frame inherits properties from the original data frame. 
}

a_names <- destroyX(a_names)

```


```{r 2.3_in_book,echo=FALSE}
...
```{r 2.3_btb_final, echo=FALSE}

a_names %>% 
  filter(sex == "F") %>% 
  select(-X, -sex) %>% 
  # select(name:`1950`) %>% 
  pivot_longer(-name,            #This is used to rotate the data frame completely, except for the name column
               # `1880`:`1900`,
               names_to = "year",      #column names will be gathered in a new column called "year"
               values_to = "count") %>%   #column values, thus the actual data, are now in the column "count"
  mutate(last = str_sub(name, -1)) %>% 
  group_by(last, year) %>% 
  summarise(n = sum(count)) %>% 
  ungroup() %>%
  ...
  mutate(percent = 100 * n / sum(n),
         year    = as.double(year),
         last    = toupper(last),
         size    = ifelse(last %in% c("N", "A", "E", "Y"), "a", "b"),
         type    = case_when(
           last == "N" ~ "3",
           last == "A" ~ "1",
           last == "E" ~ "2",
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), limits = c(0, 40),
                     breaks = 0:2 * 20, labels = function(x) str_c(x, "%")) +
  labs(subtitle = "Last letters of girls' names",
       x = NULL,
       y = "Percentage of all girls' names that year") +
  theme_light()
...
One example could be using a wrong indicator to study a certain phenomenon. For example if I used as the only indicator of rage whether the person was frowining or not, I would reliably count the numbers of time said person might be frowing, but I still would have no indicator or their emotional state. My "research" would be reproducible, the issue lies in the indicator I have chosen. 
